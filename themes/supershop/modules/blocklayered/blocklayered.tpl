		...
		<form action="#" id="layered_form">
			...
			<div>
				...
				<div id="enabled_filters">
                    ...
                    <a href="javascript:void(0)" data-rel="remove-all" class="remove-all">{l s ='Remove all' mod='blocklayered'}</a>
							...
							{if !isset($filter.slider)}
								...
								{if $filter.filter_type == 0}
									...
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-6 {if isset($value.checked)} checked {/if}">
											...
											<label 
												for="layered_{$filter.type_lite}_{$id_value}"
												{if !$value.nbr} 
													class="disabled"
												{else}
													{if isset($filter.is_color_group) && $filter.is_color_group} 
														class="layered_color {if isset($value.checked) && $value.checked}on{/if}" data-rel="{$id_value}_{$filter.id_key}"
													{/if}
												{/if}
											>
												...
												<a href="{$value.link}"{if $value.rel|trim != ''} data-rel="{$value.rel}"{/if}>
													...
													{$value.name|escape:'html':'UTF-8'}
														{if (($filter.type == 'id_attribute_group') && (isset($filter.is_color_group) && !$filter.is_color_group))} </span>{/if}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}</a>
												{*}
												{/if}

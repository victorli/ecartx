...
{if !$opc}
			<p class="cart_navigation clearfix">
				...
				{if $back}
					<a href="{$link->getPageLink('order', true, NULL, "step=0&amp;back={$back}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btnsns"><i class="fa fa-chevron-left"></i>{l s='Continue Shopping'}</a>
				{else}
					<a href="{$link->getPageLink('order', true, NULL, "step=0")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="btnsns"><i class="fa fa-chevron-left"></i>{l s='Continue Shopping'}</a>
				{/if}
		        <button type="submit" name="processAddress" class="btnsns pull-right"><span>{l s='Proceed to checkout'}<i class="fa fa-chevron-right right"></i></span></button>
			</p>
		</form>
{else}
	</div>

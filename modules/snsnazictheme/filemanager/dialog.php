...
if(isset($_GET['ad_basename']))
	$ad_basename = strip_tags(preg_replace("/[^a-zA-Z0-9\.\[\]_| -]/", '', $_GET['ad_basename']));
else die();
...
//view type
	...
	<head>
		...
		<script type="text/javascript">
			...
			var ad_basename = "<?php echo $ad_basename; ?>";
		</script>
		...
		<script>
			...
			Dropzone.options.myAwesomeDropzone = {
				...
				url: "upload.php?ad_basename="+ad_basename,
			...
			if (image_editor) {
				var featherEditor = new Aviary.Feather({
					...
					onSave: function (imageID, newURL) {
						...
						$.ajax({
							type: "POST",
							url: "ajax_calls.php?ad_basename=<?php echo $ad_basename; ?>&action=save_img",
	...
	<li class="ff-item-type-<?php echo Tools::safeOutput($class_ext); ?> file" data-name="<?php echo Tools::safeOutput($file); ?>">
		...
		} else
		{
			...
			<a href="javascript:void('')" class="link" data-file="<?php echo Tools::safeOutput($file); ?>" data-field_id="<?php echo $_GET['field_id'];?>" data-function="<?php echo Tools::safeOutput($apply); ?>">
				<div class="img-precontainer">
			...
			<figcaption>
				<form action="force_download.php" method="post" class="download-form" id="form<?php echo Tools::safeOutput($nu); ?>">
					<input type="hidden" name="ad_basename" value="<?php echo $ad_basename ?>"/>
					<input type="hidden" name="path" value="<?php echo Tools::safeOutput($subfolder.$subdir) ?>"/>
					...
					} elseif (($is_video || $is_audio) && in_array($extension_lower, $jplayer_ext))
					{
						...
						} else
						{
						   ...
						   title="<?php echo lang_Preview ?>" data-url="ajax_calls.php?ad_basename=<?php echo $ad_basename; ?>action=media_preview&title=<?php echo Tools::safeOutput($filename); ?>&file=<?php echo Tools::safeOutput($current_path.$subfolder.$subdir.$file); ?>"

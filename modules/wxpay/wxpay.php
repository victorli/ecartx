...
The user can complete the rapid payment process through mobile. WxPay allows to bind the bank card\'s fast payment basis and to provide users with safe, fast and efficient payment services.' );
		...
		$this->orderStatus = array (
				'AWAITING_WEIXIN_PAYMENT' => array (
						'color' => '#4169E1',
						'unremovable' => 1,
						'name' => $this->l ( 'Waiting to pay by Weixin' ),
						'send_email' => false 
				) 
		);
	}
	...
	public function install() {
		...
		return parent::install () && $this->registerHook ( 'header' ) && $this->registerHook ( 'backOfficeHeader' ) && $this->registerHook ( 'payment' ) && $this->registerHook ( 'paymentReturn' ) && $this->registerHook ( $admin_order_hook ) && $this->_addOrderStatus ();
	...
	protected function getConfigFormValues() {
		return array (
				'WXPAY_LIVE_MODE' => Configuration::get ( 'WXPAY_LIVE_MODE', false ),

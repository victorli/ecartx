    ...
    static function deleteGroup(){
		...
		if($item){
			if(DB::getInstance()->execute("Delete From "._DB_PREFIX_."flexiblebrand_groups Where id = ".$itemId)){
				DB::getInstance()->execute("Delete From "._DB_PREFIX_."flexiblebrand_group_lang Where group_id = $itemId");
				if($item['icon'] && file_exists($module->iconPath.$item['icon'])) unlink($module->iconPath.$item['icon']);
				DB::getInstance()->execute("Delete From "._DB_PREFIX_."flexiblebrand_group_products Where group_id = $itemId");
				$response->status = 1;

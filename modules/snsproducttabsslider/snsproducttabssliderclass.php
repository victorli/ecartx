...
class SNSProductTabsSliderClass extends Module {
	protected $_html = '';
	...
	protected function getMenuItems() {
		$conf = Configuration::get('SNSTS_CATEGORY_TABS_ID');
		if (strlen($conf))
			return explode(',', Configuration::get('SNSTS_CATEGORY_TABS_ID'));
		else
			return array();
	}
	...
	public  function _getProducts($id_category = false, $field_filter = null,  $countProduct = false )
	{
		...
		$limit = Tools::getValue('nbload') ? (int)Tools::getValue('nbload') : (int)(Configuration::get('SNSTS_NUMDISPLAY'));
		...
		$order_by = (Configuration::get('SNSTS_ORDERBY')) ? Configuration::get('SNSTS_ORDERBY') : 'date_upd';
		$order_way = (Configuration::get('SNSTS_ORDERWAY')) ? Configuration::get('SNSTS_ORDERWAY') : 'DESC';
	...
	public static function getPricesDrop($countProduct = false)
	{
		...
		$ids_product = SNSProductTabsSliderClass::_getProductIdByDate((!$beginning ? $current_date : $beginning), (!$ending ? $current_date : $ending), $context);
	...
	public static function getFeaturedProducts($countProduct = false) {
		...
		$product_ids = Configuration::get('SNSTS_PRDIDS');
	...
	public  function _getFieldProducts($field_filter = null, $countProduct = false ) {
		...
		if($field_filter == 'special_product') {
			$products = SNSProductTabsSliderClass::getPricesDrop($countProduct);
		} elseif($field_filter == 'top_sellers') {
			$products = SNSProductTabsSliderClass::getBestSalesLight($countProduct);
		} elseif($field_filter == 'new_product') {
			$products = SNSProductTabsSliderClass::getNewProducts($countProduct);
		} elseif($field_filter == 'featured_product') {
			$products = SNSProductTabsSliderClass::getFeaturedProducts($countProduct);
		}
		return $products;
	}
...
//						}
	...
	public function snsCreateTab()
	{
		...
		$tab = new Tab();
		$tab->active = 1;
		$tab->class_name = 'AdminSNSProductTabsSlider';
		$tab->name = array();
		foreach (Language::getLanguages() as $lang)
			$tab->name[$lang['id_lang']] = 'SNS Product Tabs Slider';
	...
	public function snsDeleteTab()
	{
		$id_tab = Tab::getIdFromClassName('AdminSNSProductTabsSlider');
		$parent_tab_id = Tab::getIdFromClassName('AdminSNS');

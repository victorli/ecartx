...
<li class="category_{$node.id}{if isset($last) && $last == 'true'} last{/if}{if $node.children|@count > 0} has_child{/if}">
	<a href="{$node.link|escape:'html':'UTF-8'}" {if isset($currentCategoryId) && $node.id == $currentCategoryId}class="selected"{/if}
		...
		{if isset($node.icon) && $node.icon}<i class="fa {$node.icon}"></i>{/if}
	...
	{if $node.children|@count > 0}
		...
		{foreach from=$node.children item=child name=categoryTreeBranch}
			...
			{else}
				{include file="./category-tree-branch.tpl" node=$child last='false'}
			{/if}
		{/foreach}
		</ul>
	{/if}

...
class GroupCategory extends Module
{
	...
	*/
	...
	protected function getShowManufactory($manufactories=array()){		
		...
		if($manufactories){
			foreach($manufactories as $manufactory){
				...
				$result .= '<li id="manufacturer-'.$feature.'">
				...
				if($languages){
                	...
                	foreach($languages as $index=>$language){
						...
						if($check){
	                    	...
	                    	$insertDatas[] = array('group_id'=>$itemId, 'id_lang'=>$language->id, 'id_shop'=>$shopId, 'name'=>$name, 'banner'=>$bannerName, 'banner_link'=>$bannerLink, 'banner_size'=>'') ;
	                    }
                	}
					if($insertDatas) Db::getInstance(_PS_USE_SQL_SLAVE_)->insert('groupcategory_group_lang', $insertDatas);
                }
    ...
    public function buildModule($moduleItem, $langId, $shopId){
		...
		if (!$this->isCached('banners.'.$moduleItem['layout'].'.tpl', Tools::encrypt($cacheKey))){
		...
		if($moduleItem['cat_type'] == 'auto'){
			if($moduleItem['order_by'] == 'view' || $moduleItem['order_by'] == 'review' || $moduleItem['order_by'] == 'rate'){			
				...
				$products = $this->frontGetProducts($moduleCategoryIds, $moduleItem['on_condition'], $moduleItem['on_sale'], $moduleItem['on_new'], $moduleItem['on_discount'], $langId, 0, $moduleItem['max_item'], $moduleItem['order_by'], $moduleItem['order_way']);
				if($products){
					$this->context->smarty->assign('products', $products);
					$sectionContent .= $this->display(__FILE__, 'product-list-nocache.'.$moduleItem['layout'].'.tpl');			
				}else{
					$sectionContent .= '<div>'.$this->l('No product!').'</div>';
				}
				$sectionContent .='</div>';
				unset($products);
			}else{
				$cacheFile = 'product-list.'.$moduleItem['layout'].'.tpl';			
				$products = array();
				if ($moduleItem['is_cache'] == '0' || !$this->isCached($cacheFile, Tools::encrypt($cacheKey))){
					...
					if($products){
						...
						$this->context->smarty->assign('products', array());
					}
				}			
				$sectionContent	.='<div id="product-list-'.$moduleItem['id'].'-0-0" class="lazy-carousel check-active active tab-content-'.$moduleItem['id'].'-0-0">';
			...
			if ($moduleItem['is_cache'] == '0' || !$this->isCached($cacheFile, Tools::encrypt($cacheKey))){
				if($params->products && count($params->products) >0){
					foreach($params->products as $productId){
						if($productId >0)
	                    	$products[] = $this->frontGetProductById($productId, $langId);		                    
	                } 
				}	
			}			
			$sectionContent	.='<div id="product-list-'.$moduleItem['id'].'-0-0" class="lazy-carousel check-active active tab-content-'.$moduleItem['id'].'-0-0">';
		...
		if(isset($params->features) && $params->features){			
			foreach ($params->features as $feature) {
				...
				if($feature == 'view' || $feature == 'review' || $feature == 'rate'){			
					$sectionContent	.='<div id="product-list-'.$moduleItem['id'].'-'.$feature.'-0" class="lazy-carousel check-active tab-content-'.$moduleItem['id'].'-'.$feature.'-0">';
					$products = $this->frontGetProducts($moduleCategoryIds, $moduleItem['on_condition'], $moduleItem['on_sale'], $moduleItem['on_new'], $moduleItem['on_discount'], $langId, 0, $moduleItem['max_item'], $feature, $moduleItem['order_way']);
					if($products){
						$this->context->smarty->assign('products', $products);
						$sectionContent .= $this->display(__FILE__, 'product-list-nocache.'.$moduleItem['layout'].'.tpl');			
					}else{
					...
					if ($moduleItem['is_cache'] == '0' || !$this->isCached($cacheFile, Tools::encrypt($cacheKey.'|'.$feature))){
						if($feature == 'arrival')							
							$products = $this->frontGetProducts($moduleCategoryIds, $moduleItem['on_condition'], $moduleItem['on_sale'], 1, $moduleItem['on_discount'], $langId, 0, $moduleItem['max_item'], 'date_add', $moduleItem['order_way']);
						else if($feature == 'special')
							$products = $this->frontGetProducts($moduleCategoryIds, $moduleItem['on_condition'], $moduleItem['on_sale'], $moduleItem['on_new'], 1, $langId, 0, $moduleItem['max_item'], 'discount', $moduleItem['order_way']);
						else 
							$products = $this->frontGetProducts($moduleCategoryIds, $moduleItem['on_condition'], $moduleItem['on_sale'], $moduleItem['on_new'], $moduleItem['on_discount'], $langId, 0, $moduleItem['max_item'], $moduleItem['order_by'], $moduleItem['order_way']);
						if($products){
							$this->context->smarty->assign('products', $products);
		...
		$sectionFeatures .='</ul>';		
		$cacheFile = 'manufacturer.'.$moduleItem['layout'].'.tpl';
            ...
            foreach($items as $item){
				...
				if($item['cat_type'] == 'auto'){
					if($item['order_by'] == 'view' || $item['order_by'] == 'review' || $item['order_by'] == 'rate'){			
						...
						$products = $this->frontGetProducts($arrSubCategory, $item['on_condition'], $item['on_sale'], $item['on_new'], $item['on_discount'], $langId, 0, $item['max_item'], $item['order_by'], $item['order_way']);
						if($products){
							$this->context->smarty->assign('products', $products);
							$sectionContent .= $this->display(__FILE__, 'product-list-nocache.'.$moduleItem['layout'].'.tpl');			
						}else{
		...
		$sectionBanners .= $this->display(__FILE__, 'banners.'.$moduleItem['layout'].'.tpl', Tools::encrypt($cacheKey));
		/*
		print_r($params);
		die;
		...
		print_r($moduleItem);
		die;
		echo $moduleItem['layout'].'.option'.$themeOption.'.layout.tpl';
		die;
		print_r($moduleItem);
		die;
        //$themeOption = 3;
    ...
    function clearCache(){
		...
		if($this->arrLayout)
			foreach ($this->arrLayout as $key => $value){
				parent::_clearCache('manufacturer.default.tpl');
				parent::_clearCache('product-list-nocache.default.tpl');
				parent::_clearCache('product-list.default.tpl');
				parent::_clearCache('sublinks.default.tpl');
				parent::_clearCache('banners.default.tpl');
				parent::_clearCache('default.option1.layout.tpl');
			} 		
	}
